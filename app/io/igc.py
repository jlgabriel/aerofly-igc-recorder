...
class IGCWriter:
    ...
    def _write_header(self) -> bool:
        ...
        try:
            ...
            self.writer.write_date(self.start_time.date())
            
            # Write basic header records
            if self.pilot_name:
                self.writer.write_pilot(self.pilot_name)
                
            if self.glider_type:
                self.writer.write_glider_type(self.glider_type)
                
            if self.glider_id:
                self.writer.write_glider_id(self.glider_id)
                
            # Write logger info
            self.writer.write_logger_id(
                manufacturer=IGC_MANUFACTURER_CODE,
                id=IGC_LOGGER_ID
            )
            self.writer.write_firmware_version("1.0")
            self.writer.write_hardware_version("1.0")
            
            # Write GPS receiver info
            self.writer.write_gps_receiver(
                "Aerofly FS4 Simulator",
                "SIMULATOR",
                "None"
            )
